...
def jenere_slug(chenn):
    ...
    for karaktè in chenn:
        if karaktè.isalnum() or karaktè == '-':
            slug += karaktè
        elif karaktè.isspace():
            slug += separe

    # Elimine espas oswa karaktè "-" nan debi ak fen chenn la
    slug = slug.strip(separe)
    
    # Remake plizyè karaktè "-" nan yon sèl karaktè
    slug = '-'.join(slug.split(separe))
    
    # Konvèti nan ti lèt bwa majiskil
    slug = slug.lower()

    return slug

# Echantiyon
chenn = "Hello, World! This is an Example Slug."
